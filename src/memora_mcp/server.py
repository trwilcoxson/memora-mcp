...
def memory_save(content: str, kind: str = "note") -> str:
    ...
    client = _get_client()
    with contextlib.redirect_stdout(sys.stderr):
        entries = client.add(content, type="chat" if kind == "conversation" else "doc")
    if not entries:
        return "Nothing was stored (the extractor found no durable facts)."
    return f"Stored {len(entries)} memories:\n" + "\n".join(_entry_lines(entries))
