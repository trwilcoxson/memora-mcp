...
| **Distill** | One background turn on your model plane extracts the durable facts + cue anchors; secrets and noise are dropped first. | subscription CLI / API / gateway — detected, not configured |
...
- **Model plane is detected, not configured.** Logged-in `claude` or `codex` → your subscription (default, no key). `OPENAI_API_KEY` or a gateway → that, if you prefer. Nothing available → memory still stores and recalls; distillation waits until a plane exists.
- **Embeddings never require a service.** Recall runs on a bundled in-process model (no key, no daemon) by default; a served embedding endpoint is an opt-in upgrade, never a prerequisite.
